...
def store_token_in_database(token, user_email):
    ...
    token_record = TokenForRegister(token=token, user_email=user_email, expiration_date=datetime.datetime.now() + datetime.timedelta(hours=24))
...
def verify_token(token):
    ...
    if not token_record or token_record.expiration_date < datetime.datetime.now() or token_record.used:
        return False
